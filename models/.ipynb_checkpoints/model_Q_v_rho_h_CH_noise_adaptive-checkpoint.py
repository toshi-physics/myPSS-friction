...
def main():
    ...
    # Define Gamma
    system.create_term("Gamma", [("rho_end_rho", (np.tanh, None))], [Gamma0, 0, 0, 0])
    ...
    system.create_term("rho", [("Jy_noise", None)], [-1, 0, 0, 1])
        # growth
    system.create_term("rho", [("rho", None), ("rho_end", (np.power, -1)), ("rho_end_rho", None)], [1, 0, 0, 0])
        # pressure
    ...
    t=0
    while t<T:
        dt_next = system.update_system(dt)
